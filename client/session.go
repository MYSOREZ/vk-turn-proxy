...
// NullLoggerFactory suppresses pion logs
...
func RunSession(
	...
	log.Printf("[ВОРКЕР #%d] [DTLS] Рукопожатие...", sessionID)
	...
	if err != nil {
		if useWrap {
			...
			if strings.Contains(errStr, "deadline") || strings.Contains(errStr, "timeout") {
				return false, fmt.Errorf("WRAP_AUTH_TIMEOUT: DTLS timeout, пароль не подтверждён")
			}
		}
	...
	if getConfig && configCh != nil {
		...
		if confErr != nil {
			...
			log.Printf("[ВОРКЕР #%d] Сервер не выдал WireGuard-конфиг", sessionID)
		}
	} else {
